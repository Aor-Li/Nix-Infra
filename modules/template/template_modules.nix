/*
  ...
  3. 模块命名
    ...
    当然命名是必不可少的，上面同一功能的三种命名可以用同一个。问题是：
      
      1. 如果直接命名，A/C, B/C分别定义了A和B功能中的一部分，C可能会重名。 

      2. 如果用A/C，B/C作为的路径作为名字，对于当前配置系统的模块是可行的。但作为导出模块时，莫名附带了其父功能(A或者B)的名字，这对于外部使用C模块似乎不合理
